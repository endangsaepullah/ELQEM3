"""styles.py — Global CSS, header, footer, UI helpers."""
...
def apply_global_style():
    ...
    h1,h2,h3,h4 {{ font-family:'Rajdhani',sans-serif !important; color:var(--text) !important; }}
    ...
    .stTabs [data-baseweb="tab"] {{ background:transparent; color:var(--muted); font-family:'Rajdhani',sans-serif; font-weight:600; border-radius:6px; }}
    .stTabs [aria-selected="true"] {{ background:rgba(0,212,255,0.12) !important; color:var(--accent) !important; }}
    .stButton>button {{ background:rgba(0,212,255,0.07); border:1px solid rgba(0,212,255,0.3); color:var(--accent); font-family:'Rajdhani',sans-serif; font-weight:600; border-radius:6px; transition:all .2s; }}
    ...
    .stTextInput>div>div>input,.stTextArea>div>div>textarea,.stSelectbox>div>div {{ background:var(--bg2) !important; border:1px solid var(--border) !important; border-radius:6px !important; color:var(--text) !important; }}
...
def render_footer():
    ...
    cols = [
        ("Modul Evaluasi", ["Dashboard Utama","ISO 9001","IATF 16949","Engineering Lifecycle","Konsistensi Mutu","Evaluasi Batch"]),
        ("Analisis & Laporan", ["Integrated Quality Score","Analisis Mutu MAUNG MV3","Simulasi What-If","Kesimpulan & Hipotesis","Data Wawancara"]),
        ("Platform", ["About Platform","Teori & Referensi","Manajemen User","Pengaturan Platform"]),
        ("Tentang", ["Endang Saefullah, ST, CLA","Universitas Pertahanan RI","PT Pindad (Persero)","Magister Teknik Industri Pertahanan"]),
    ]
    col_divs = ""
    for title, items in cols:
        ...
        col_divs += (
            ...
            f'<div style="font-family:Rajdhani;font-size:.6rem;font-weight:700;color:{accent};letter-spacing:2px;text-transform:uppercase;margin-bottom:.5rem;padding-bottom:.3rem;border-bottom:1px solid #21262d;">{title}</div>'
            + rows + '</div>'
        )
    st.markdown(
        '<div style="margin-top:3rem;background:#0a0e1a;border-top:1px solid #21262d;padding:1.5rem 2rem 0;">'
        f'<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1.5rem;margin-bottom:1.25rem;">{col_divs}</div>'
        '<div style="border-top:1px solid #21262d;padding:.65rem 0;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.35rem;">'
        f'<span style="font-family:Rajdhani;font-size:.68rem;color:{accent};font-weight:600;letter-spacing:1px;">IQLE Platform &nbsp;&middot;&nbsp; Quality 4.0 Dashboard</span>'
        f'<span style="font-size:.62rem;color:#3d5470;font-family:Rajdhani;">PT Pindad &nbsp;&middot;&nbsp; Universitas Pertahanan RI &nbsp;&middot;&nbsp; &copy; {year}</span>'
        '</div></div>',
        unsafe_allow_html=True
    )
...
def score_bar(label, value, color="#00d4ff"):
    st.markdown(
        f'<div style="margin-bottom:.7rem;padding:.75rem 1rem;background:#111827;border:1px solid {color}22;border-left:3px solid {color};border-radius:8px;">'
        f'<div style="display:flex;justify-content:space-between;align-items:center;">'
        f'<span style="font-size:.8rem;color:#7a9bb5;">{label}</span>'
        f'<span style="font-family:Rajdhani;font-size:1.15rem;font-weight:700;color:{color};">{value:.1f}</span>'
        f'</div>'
...
def plotly_layout():
    return dict(
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(13,19,33,0.5)',
