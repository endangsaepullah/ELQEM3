...
def render_footer():
    """Simple text-only sitemap footer — no buttons, no key conflicts."""
    import streamlit as st
    line1  = get_setting("footer_line1", "IQLE Platform · Prototype Akademik Magister Teknik · Endang Saefullah, ST, CLA")
    ...
    sitemap = {
        ...
        "Tentang": [
            "Peneliti: Endang Saefullah, ST, CLA",
            "Universitas Pertahanan RI (UNHAN)",
            "PT Pindad (Persero)",
    ...
    cols_map = ["  ".join(
        f'<span style="color:#3d5470;">&#8250;</span> '
        f'<span style="color:#4a6fa5;font-size:.72rem;">{item}</span>'
        for item in items
    ) for items in sitemap.values()]

    cols_html = "".join(
        f'<div style="flex:1;min-width:180px;padding:0 1rem;">'
        f'<div style="font-size:.6rem;color:{accent};letter-spacing:2px;'
        f'text-transform:uppercase;font-weight:700;margin-bottom:.6rem;">{title}</div>'
        + "".join(
            f'<div style="padding:2px 0;color:#4a6fa5;font-size:.72rem;">'
            f'<span style="color:#2a3f55;">&#8250;</span> {item}</div>'
            for item in items
        ) +
        f'</div>'
        for title, items in sitemap.items()
    )

    st.markdown(
        f'<div style="margin-top:3rem;background:#0a0e1a;'
        f'border-top:1px solid rgba(0,212,255,0.1);">'

        # Sitemap
        f'<div style="padding:1.5rem 2rem 1rem;display:flex;flex-wrap:wrap;gap:1rem;">'
        + cols_html +
        f'</div>'

        # Bottom bar
        f'<div style="padding:.65rem 2rem;background:#070b14;'
        f'border-top:1px solid rgba(0,212,255,0.06);'
        f'display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.5rem;">'
        f'<div style="font-size:.68rem;color:#3d5470;">'
        f'<span style="color:{accent};font-family:Rajdhani;font-weight:600;">{line1}</span>'
        f'</div>'
        f'<div style="font-size:.65rem;color:#2a3f55;font-family:Rajdhani;">'
        f'{line2} &nbsp;·&nbsp; &copy; {year}'
        f'</div></div></div>',
        unsafe_allow_html=True
    )
